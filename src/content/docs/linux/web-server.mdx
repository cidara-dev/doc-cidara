...
```
### Change PHP Version on your machine
For change version of your PHP,enter Command on your Terminal (Ex: change to php7.4)
...
```sh
    MariaDB> GRANT ALL PRIVILEGES ON *.* TO 'exampleuser'@'localhost';
```
...
```sh
    $ sudo wget -P Downloads https://www.phpmyadmin.net/downloads/phpMyAdmin-5.2.1-english.tar.gz
```
##### Download Keyring
```sh
...
```sh
    $ sudo wget https://www.phpmyadmin.net/downloads/phpMyAdmin-5.2.1-english.tar.gz.asc
